...
interface NormalizationCatalogData {
  rules?: NormalizationRule[];
}

interface NormalizationCatalogProps {
  onClose?: () => void;
}

const NormalizationCatalog: React.FC<NormalizationCatalogProps> = ({ onClose }) => {
  const queryClient = useQueryClient();

  const { data: catalogData, isLoading } = useQuery<{ data: NormalizationCatalogData }>({
    queryKey: ['normalization-catalog'],
    queryFn: () => itamAPI.dataQuality.getNormalizationCatalog(),
  });

  const updateMutation = useMutation({
    mutationFn: (data: NormalizationCatalogData) => itamAPI.dataQuality.updateNormalizationCatalog(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['normalization-catalog'] });
