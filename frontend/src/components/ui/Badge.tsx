 ...
 */
const Badge: React.FC<BadgeProps> = memo(({ children, variant = 'gray', size = 'md' }) => {
  const variants = {
 ...
 */
export const getStatusVariant = (status: string): 'success' | 'warning' | 'danger' | 'info' | 'gray' => {
 ...
 */
export const getConditionVariant = (condition: string): 'success' | 'warning' | 'danger' | 'info' | 'gray' => {
  const conditionMap: Record<string, 'success' | 'warning' | 'danger' | 'info' | 'gray'> = {
    excellent: 'success',
    good: 'info',
    fair: 'warning',
    poor: 'danger',
