...
interface Integration {
  ...
  name?: string;
  description?: string;
  authType?: 'api-key' | 'oauth' | 'basic-auth';
  endpoint?: string;
  scopes?: string[];
}
...
const IntegrationConfigModal = ({ isOpen, onClose, integration }: IntegrationConfigModalProps) => {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: FormData) => integrationConfigsAPI.createOrUpdate(data),
    onSuccess: () => {
  ...
  const testMutation = useMutation({
    mutationFn: (name: string) => integrationConfigsAPI.test(name),
  ...
  const handleOAuthConnect = async () => {
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to initiate OAuth flow');
    }
  };
          ...
          <div className="space-y-3">
            ...
            {formData.syncSettings.autoSync && (
              <div>
                ...
                onChange={(e) => setFormData({
                  ...formData,
                  syncSettings: { ...formData.syncSettings, syncInterval: parseInt(e.target.value, 10) }
                })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                />
