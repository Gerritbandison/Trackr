...
const DepreciationCard = ({ asset }: DepreciationCardProps) => {
  const [selectedMethod, setSelectedMethod] = useState<string>(DEPRECIATION_METHODS.STRAIGHT_LINE);
  const [showSchedule, setShowSchedule] = useState(false);

  const purchasePrice = asset.purchase?.unitCost;
  const purchaseDate = asset.purchase?.date;
  ...
  const assetWithDepreciation = {
    ...asset,
    purchasePrice,
    purchaseDate,
    usefulLife: asset.usefulLife || 4,
    salvageValue: asset.salvageValue || 0,
  };

  const depreciationData = calculateAssetDepreciation(assetWithDepreciation, selectedMethod);
  const schedule = getDepreciationSchedule(assetWithDepreciation, selectedMethod);
