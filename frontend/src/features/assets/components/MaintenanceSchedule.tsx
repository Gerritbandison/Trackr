 ...
 */
const MaintenanceSchedule = ({ asset }: MaintenanceScheduleProps) => {
  const [showAddModal, setShowAddModal] = useState(false);
  const [editingMaintenance, setEditingMaintenance] = useState<MaintenanceRecord | null>(null);
  const queryClient = useQueryClient();

  // Calculate maintenance schedule
  const interval = getMaintenanceInterval(asset?.class, asset?.model);
  const lastMaintenanceDate = asset?.custom?.lastMaintenanceDate 
    ? new Date(asset.custom.lastMaintenanceDate as string) 
    : asset?.purchase?.date 
    ? new Date(asset.purchase.date)
    : null;
  ...
  const estimatedCost = asset?.purchase?.unitCost
    ? estimateMaintenanceCost(asset.class, asset.purchase.unitCost)
    : 0;
  const maintenanceTasks = getMaintenanceTasks(asset?.class);

  // Mock maintenance history (in production, this would come from API)
  const [maintenanceHistory, setMaintenanceHistory] = useState<MaintenanceRecord[]>(
    (asset?.custom?.maintenanceHistory as MaintenanceRecord[]) || []
  );
  ...
  const handleAddMaintenance = (maintenanceData: MaintenanceFormData) => {
    const newMaintenance: MaintenanceRecord = {
      ...
      cost: typeof maintenanceData.cost === 'string' ? parseFloat(maintenanceData.cost) || 0 : maintenanceData.cost,
    ...
    // await assetsAPI.addMaintenance(asset.globalAssetId, newMaintenance);
  };
  ...
  const getStatusColor = (status: string): 'red' | 'amber' | 'green' | 'slate' => {
    const colors: Record<string, 'red' | 'amber' | 'green' | 'slate'> = {
      overdue: 'red',
      due: 'red',
      upcoming: 'amber',
      scheduled: 'green',
...
const MaintenanceForm = ({ asset, maintenance, interval, onSave, onCancel }: MaintenanceFormProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!submitData.nextMaintenanceDate) {
      // Calculate next maintenance date
      const maintenanceDate = new Date(submitData.date);
      const nextDate = new Date(
        maintenanceDate.getTime() + interval * 24 * 60 * 60 * 1000
      );
      submitData.nextMaintenanceDate = nextDate.toISOString().split('T')[0];
    }

    onSave({
      ...submitData,
      cost: submitData.cost ? (typeof submitData.cost === 'string' ? parseFloat(submitData.cost) : submitData.cost) : 0,
    });
  };
