 ...
 */
const QRCodeGenerator = ({ asset, size = 200, title, showAssetInfo = true }) => {
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        link.href = url;
        link.download = `asset-qr-${asset?.assetTag || asset?._id}.png`;
        link.click();
  ...
  return (
    <>
      <div className="flex flex-col items-center gap-4 p-6 bg-white border border-gray-200 rounded-lg print-container">
        <h3 className="font-semibold text-gray-900">{title || `${asset.name} - QR Code`}</h3>
