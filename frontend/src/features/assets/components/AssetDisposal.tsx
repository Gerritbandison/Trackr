...
interface DisposalData {
  ...
  dataWipeStatus: 'pending' | 'in-progress' | 'completed' | 'verified';
  dataWipeMethod: string;
  dataWipeVerified: boolean;
  disposalCertificate: File | null;
  disposedBy: string;
  notes: string;
}
 ...
 */
const AssetDisposal = ({ asset, onDisposeComplete }: AssetDisposalProps) => {
  ...
  const disposeMutation = useMutation({
    mutationFn: (data: DisposalData) => {
      // In production, this would call assetsAPI.dispose(asset.globalAssetId, data)
      // For now, we'll update the asset status
      return assetsAPI.update(asset.globalAssetId, {
        ...asset,
        state: 'Disposed',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['asset', asset.globalAssetId] });
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDisposalData({ ...disposalData, dataWipeStatus: e.target.value as DisposalData['dataWipeStatus'] })
                  }
                  className="input"
                >
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 rounded-lg p-4 space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-semibold text-slate-900">{asset.name || asset.model || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
