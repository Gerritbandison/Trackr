...
import WarrantyTimeline from '../../features/assets/components/WarrantyTimeline';
import DepreciationCard from '../../features/assets/components/DepreciationCard';
import EOLCard from '../../features/assets/components/EOLCard';
import EOLBadge from '../../features/assets/components/EOLBadge';
import QRCodeGenerator from '../../features/assets/components/QRCodeGenerator';
import MaintenanceSchedule from '../../features/assets/components/MaintenanceSchedule';
import AssetDisposal from '../../features/assets/components/AssetDisposal';
import AssetHistory from '../../features/assets/components/AssetHistory';
...
const AssetDetails = () => {
  ...
  const timelineEvents = [
    ...
    asset.warrantyExpiry && {
      icon: FiShield,
      color: isValidDate(asset.warrantyExpiry) && new Date(asset.warrantyExpiry) < new Date() ? 'red' : 'amber',
      title: 'Warranty Status',
      date: asset.warrantyExpiry,
      details: isValidDate(asset.warrantyExpiry) && new Date(asset.warrantyExpiry) < new Date() ? 'Expired' : 'Active',
    },
  ].filter(Boolean);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="card hover:shadow-md transition-shadow">
              <div className="card-body p-4">
                ...
                <p className="font-semibold text-secondary-900">
                  {asset.warrantyExpiry ? (
                    isValidDate(asset.warrantyExpiry) && new Date(asset.warrantyExpiry) > new Date() ? '✓ Active' : '✗ Expired'
                  ) : 'N/A'}
                </p>
              </div>
            </div>
            ...
            <div className="card-body">
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="font-semibold text-secondary-900">
                    {safeFormatDate(asset.purchaseDate)}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  {asset.warrantyExpiry && (
                    <Badge variant={isValidDate(asset.warrantyExpiry) && new Date(asset.warrantyExpiry) < new Date() ? 'danger' : 'success'}>
                      {isValidDate(asset.warrantyExpiry) && new Date(asset.warrantyExpiry) < new Date() ? 'Expired' : 'Active'}
                    </Badge>
                  )}
                </div>
              </div>
          ...
          <div className="card">
            <div className="card-header">
              <h3 className="text-xl font-semibold text-secondary-900">Asset History</h3>
              <p className="text-sm text-secondary-600 mt-1">Complete audit trail of changes</p>
            </div>
            <div className="card-body">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-4 p-3 bg-gradient-to-r from-purple-50 to-transparent rounded-lg">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl shadow-md">
                      {asset.assignedTo?.name?.charAt(0)?.toUpperCase() || 'U'}
                    </div>
                  ...
                  {asset.assignedDate && (
                    <div className="flex items-center gap-2 text-sm text-secondary-600 mb-4 px-3">
                      ...
                      <span>Assigned on {safeFormatDate(asset.assignedDate)}</span>
                    </div>
                  )}
                  {canManage() && (
                    <button
            ...
            <div className="card-body space-y-4">
              <div>
                ...
                <p className="text-xs text-secondary-500">
                  {safeFormatDate(asset.createdAt, 'HH:mm:ss')}
                </p>
              </div>
              <div>
                <p className="text-sm text-secondary-500 uppercase tracking-wide mb-1">Last Updated</p>
                <p className="text-sm font-semibold text-secondary-900">
                  {safeFormatDate(asset.updatedAt)}
                </p>
                <p className="text-xs text-secondary-500">
                  {safeFormatDate(asset.updatedAt, 'HH:mm:ss')}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-4">
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <p className="font-semibold text-gray-900">
                  {warrantyData.data.warrantyStart 
                    ? safeFormatDate(warrantyData.data.warrantyStart)
                    : 'N/A'}
                </p>
              </div>
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <p className="font-semibold text-gray-900">
                  {warrantyData.data.warrantyEnd 
                    ? safeFormatDate(warrantyData.data.warrantyEnd)
                    : 'N/A'}
                </p>
              </div>
            </div>
            ...
            <div className="text-xs text-gray-500 text-center pt-2 border-t">
              Data retrieved from {warrantyData.source} at {safeFormatDate(warrantyData.fetchedAt, 'MMM dd, yyyy HH:mm:ss')}
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mt-4">
              ...
              <ul className="text-xs text-blue-800 space-y-1">
                ...
                {warrantyData.data.warrantyStart && <li>• Purchase Date → {safeFormatDate(warrantyData.data.warrantyStart)}</li>}
                {warrantyData.data.warrantyStart && <li>• Warranty Start → {safeFormatDate(warrantyData.data.warrantyStart)}</li>}
                {warrantyData.data.warrantyEnd && <li>• Warranty End → {safeFormatDate(warrantyData.data.warrantyEnd)}</li>}
