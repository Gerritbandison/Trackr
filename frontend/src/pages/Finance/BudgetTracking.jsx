...
const BudgetTracking = () => {
  ...
  const [budgets, setBudgets] = useState(() => {
    // Initialize budgets from localStorage, or empty object if none saved
  ...
  // Initialize default budgets when departmentsData is loaded
  useEffect(() => {
    ...
    const saved = localStorage.getItem('department-budgets');
    if (saved) {
      // If budgets already exist in localStorage, don't overwrite
      return;
    }

    // Create default budgets for all departments
    ...
    // Only set if we created new budgets (state was empty)
    setBudgets((prevBudgets) => {
      if (Object.keys(prevBudgets).length === 0) {
        localStorage.setItem('department-budgets', JSON.stringify(defaultBudgets));
