...
const ContractDetails = () => {
  ...
  // Check if editing contract
  const isEditingContract = location.pathname.includes('/edit');

  if (isNewContract || (isEditingContract && contract)) {
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center gap-4">
          <button
            onClick={() => contract ? navigate(`/contracts/${contract._id}`) : navigate('/contracts')}
          ...
          <div>
            <h1 className="text-4xl font-bold text-secondary-900 tracking-tight">
              {isEditingContract ? 'Edit Contract' : 'Create New Contract'}
            </h1>
            <p className="text-secondary-600 mt-2">
              {isEditingContract ? 'Update contract information' : 'Add a new contract to the system'}
            </p>
          </div>
        </div>
        <div className="card">
          <div className="card-body">
            <ContractForm
              contract={isEditingContract ? contract : null}
              vendorId={isEditingContract ? contract?.vendorId || contract?.vendor?._id : vendorIdFromState}
              onClose={() => contract ? navigate(`/contracts/${contract._id}`) : navigate('/contracts')}
              onSuccess={(newContract) => {
                if (newContract?._id) {
                  navigate(`/contracts/${newContract._id}`);
                } else {
                  navigate('/contracts');
                }
              }}
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            ...
            <div className="card-body space-y-3">
              <button 
                onClick={() => {
                  if (contract.documents && contract.documents.length > 0) {
                    ...
                    contractsAPI.downloadDocument(id, firstDoc.id).catch(() => {
              ...
              {vendor?.contactEmail && (
                <a
                  href={`mailto:${vendor.contactEmail}?subject=Contract: ${contract.name}`}
                  className="w-full btn btn-outline flex items-center justify-center gap-2"
                >
                  <FiMail size={16} />
                  Email Vendor
